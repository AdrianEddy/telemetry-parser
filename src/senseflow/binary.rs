...
pub fn parse<T: Read + Seek>(stream: &mut T, _size: usize) -> Result<Vec<SampleInfo>> {
    ...
    let s = buf[69] as u32;
    let ms = 0u32;
    let create_at = chrono::NaiveDate::from_ymd_opt(yy, mm, dd)
    ...
    let acc_odr = d.read_u16::<LittleEndian>()?;
    let acc_max_bw = d.read_u16::<LittleEndian>()?;
    let acc_timeoffset = d.read_i32::<LittleEndian>()?;
    let acc_range = d.read_u32::<LittleEndian>()? as f64;
    ...
    let mag_odr = d.read_u16::<LittleEndian>()?;
    let mag_max_bw = d.read_u16::<LittleEndian>()?;
    let mag_timeoffset = d.read_i32::<LittleEndian>()?;
    let mag_range = d.read_u32::<LittleEndian>()? as f64 / 1000.0;
    ...
    let metadata = serde_json::json!({
       ...
       "log_freq": log_freq,
       "acc_odr": acc_odr,
       "acc_max_bandwidth": acc_max_bw,
       "acc_timeoffset": acc_timeoffset,
       "acc_range": acc_range,
       "gyro_odr": gyro_odr,
       ...
       "mag_odr": mag_odr,
       "mag_max_bandwidth": mag_max_bw,
       "mag_timeoffset": mag_timeoffset,
       "mag_range": mag_range,
       "timestamp_step": timestamp_step,
       "init quad": _init_quad,
    });
    ...
    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Accelerometer, TagId::Data, "Accelerometer data", Vec_TimeVector3_f64, |v| format!("{:?}",  v), accl, vec![]),
    );
    util::insert_tag(
        &mut map,
    ...
    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Gyroscope,     TagId::Orientation, "IMU orientation", String, |v| v.to_string(), imu_orientation.into(), Vec::new()),
    );
    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Accelerometer, TagId::Orientation, "IMU orientation", String, |v| v.to_string(), imu_orientation.into(), Vec::new()),
    );

    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Magnetometer,  TagId::Data, "Magnetometer data", Vec_TimeVector3_i64f64, |v| format!("{:?}", v), magn, vec![]),
    );
    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Magnetometer,  TagId::Unit, "Magnetometer unit", String, |v| v.to_string(), "μT".into(), Vec::new()),
    );

    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Custom("Angle".into()),        TagId::Data, "Angle data", Vec_TimeVector3_f64, |v| format!("{:?}", v), angl, vec![]),
    );
    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Custom("Angle".into()),        TagId::Unit, "Angle unit", String, |v| v.to_string(), "deg".into(),  Vec::new()),
    );

    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Quaternion,   TagId::Data, "Quaternion data",   Vec_TimeQuaternion_f64,  |v| format!("{:?}", v), quat, vec![]),
    );

    util::insert_tag(
        &mut map,
        tag!(parsed GroupId::Default, TagId::Metadata, "Metadata", Json, |v| serde_json::to_string(v).unwrap(), metadata, vec![]),
    );
    Ok(vec![SampleInfo {
